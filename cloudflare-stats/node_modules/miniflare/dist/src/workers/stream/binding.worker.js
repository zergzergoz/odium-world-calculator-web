...
function rowToStreamVideo(row, entryUrl) {
  let placeholderUrl = `https://customer-placeholder.cloudflarestream.com/${row.id}`, videoUrl = `${entryUrl.origin}${CorePaths.STREAM_VIDEO}/${row.id}/watch`;
  return {
    id: row.id,
    creator: row.creator,
...
var StreamBinding = class extends WorkerEntrypoint {
  async fetch(request) {
    let match = new URL(request.url).pathname.match(/^\/__cf_local\/stream\/([^/]+)\/watch$/);
    if (!match)
