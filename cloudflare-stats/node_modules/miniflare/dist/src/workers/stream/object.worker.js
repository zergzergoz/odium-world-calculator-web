...
var BLOB_NAMESPACE = "stream-data", StreamObject = class extends DurableObject {
  ...
  constructor(state, env) {
    ...
    db.exec("PRAGMA foreign_keys = ON"), db.exec(SQL_SCHEMA), this.#db = db, this.#stmts = sqlStmts(db, () => this.#now()), this.#blob = new BlobStore(env.MINIFLARE_BLOBS, BLOB_NAMESPACE);
  }
  ...
  async generateToken(id) {
    if (get(this.#stmts.getVideo({ id })) === void 0)
      throw new NotFoundError(`Video not found: ${id}`);
    let payload = {
      sub: id,
      kid: "local-mode-key",
