  ...
  for (let { name: table, sql } of tables) {
    ...
    for (let dataRow of rows_cursor.raw()) {
      let formattedCells = dataRow.map((cell, i) => {
        ...
        return cell === null ? "NULL" : cellType === "number" ? Number.isFinite(cell) ? cell : cell > 0 ? "9e999" : "-9e999" : cellType === "string" ? outputQuotedEscapedString(cell) : cell instanceof ArrayBuffer ? `X'${Array.prototype.map.call(new Uint8Array(cell), (b) => b.toString(16).padStart(2, "0")).join("")}'` : (console.error({
