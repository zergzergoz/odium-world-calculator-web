...
}, HEADER_QUEUE_NAME = "MF-Queue-Name";
...
var QueueMessageDelaySchema = z.number().int().min(0).max(86400).optional(), QueueProducerOptionsSchema = /* @__PURE__ */ z.object({
...
), QueueProducersSchema = /* @__PURE__ */ z.record(z.string(), QueueProducerSchema), QueueConsumerOptionsSchema = /* @__PURE__ */ z.object({
  ...
  z.object({ workerName: z.string() })
), QueueConsumersSchema = /* @__PURE__ */ z.record(z.string(), QueueConsumerSchema), QueueContentTypeSchema = /* @__PURE__ */ z.enum(["text", "json", "bytes", "v8"]).default("v8"), QueueIncomingMessageSchema = /* @__PURE__ */ z.object({
...
var QueueMessage = class _QueueMessage {
  constructor(id, timestamp, body) {
    ...
    this.body = body;
    this.#bytes = _QueueMessage.#byteLength(body);
  }
  id;
  timestamp;
  body;
...
var QueueBrokerObject = class extends MiniflareDurableObject {
  ...
  #dispatchBatch(workerName, batch, metadata) {
  ...
  #flush = async () => {
    ...
    let batchSize = consumer.maxBatchSize ?? DEFAULT_BATCH_SIZE, maxAttempts = (consumer.maxRetries ?? DEFAULT_RETRIES) + 1, maxAttemptsS = maxAttempts === 1 ? "" : "s", batch = this.#messages.splice(0, batchSize);
    ...
    }, startTime = Date.now(), endTime, response;
    try {
    ...
    for (let message of batch)
      if (retryAll || retryMessages.has(message.id))
        if (failedMessages++, message.incrementFailedAttempts() < maxAttempts) {
          ...
          let fn = () => {
            this.#messages.push(message), this.#backlogBytes += message.bytes, this.#ensurePendingFlush();
    ...
    for (let message of messages) {
      let randomness = crypto.getRandomValues(new Uint8Array(16)), id = message.id ?? Buffer2.from(randomness).toString("hex"), timestamp = new Date(message.timestamp ?? this.timers.now()), body = deserialise(message), msg = new QueueMessage(id, timestamp, body), fn = () => {
        this.#messages.push(msg), this.#backlogBytes += msg.bytes, this.#ensurePendingFlush();
  ...
  // still succeeds.
  async #tryRemoteConsumer(req) {
    let proxy = this.env[QueueBindings.MAYBE_SERVICE_QUEUE_PROXY];
    if (proxy === void 0)
      return null;
    let headers = new Headers(req.headers);
    headers.set(HEADER_QUEUE_NAME, this.name);
    let delay = this.#maybeProducer?.deliveryDelay;
    delay !== void 0 && !headers.has("X-Msg-Delay-Secs") && headers.set("X-Msg-Delay-Secs", String(delay));
    try {
      let response = await proxy.fetch(req.url, {
        method: req.method,
        headers,
        body: await req.arrayBuffer()
      });
      return response.status === 503 ? (await this.logWithLevel(
        LogLevel.DEBUG,
        `Dropped message on queue "${this.name}": ${await response.text()}`
      ), null) : response;
    } catch (e) {
      return await this.logWithLevel(
        LogLevel.WARN,
  ...
  message = async (req) => {
    ...
    if (this.#maybeConsumer === void 0)
      return await this.#tryRemoteConsumer(req) ?? Response.json(messageResponse);
  ...
  batch = async (req) => {
    ...
    if (this.#maybeConsumer === void 0)
      return await this.#tryRemoteConsumer(req) ?? Response.json(batchResponse);
    ...
    return this.#enqueue(body.messages, delay), Response.json(batchResponse);
  };
  metrics = async (_req) => Response.json(this.#getMetricsResponseObject());
};
__decorateClass([
  POST("/message")
], QueueBrokerObject.prototype, "message", 2), __decorateClass([
  POST("/batch")
], QueueBrokerObject.prototype, "batch", 2), __decorateClass([
  GET("/metrics")
], QueueBrokerObject.prototype, "metrics", 2);
export {
  QueueBrokerObject
};
