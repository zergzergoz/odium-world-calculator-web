...
var RouterOuterEntrypoint = class extends WorkerEntrypoint {
  async fetch(request) {
    let loopbackCtx = this.ctx, analytics = new Analytics(this.env.ANALYTICS), inner;
    try {
      ...
      throw (this.ctx, this.env.SENTRY_DSN, this.env.SENTRY_ACCESS_CLIENT_ID, this.env.SENTRY_ACCESS_CLIENT_SECRET, this.env.COLO_METADATA, this.env.VERSION_METADATA, this.env.CONFIG?.account_id, this.env.CONFIG?.script_id, void 0)?.captureException(err), err instanceof Error && analytics.setData({ error: err.message }), analytics.write(), err;
    }
    return inner.fetch(request);
  }
}, RouterInnerEntrypoint = class extends WorkerEntrypoint {
  async fetch(request) {
    let sentry, userWorkerInvocation = !1, analytics = new Analytics(this.env.ANALYTICS), performance = new PerformanceTimer(this.env.UNSAFE_PERFORMANCE), startTimeMs = performance.now();
    try {
      this.env.JAEGER || (this.env.JAEGER = mockJaegerBinding()), sentry = (this.ctx, this.env.SENTRY_DSN, this.env.SENTRY_ACCESS_CLIENT_ID, this.env.SENTRY_ACCESS_CLIENT_SECRET, this.env.COLO_METADATA, this.env.VERSION_METADATA, this.env.CONFIG?.account_id, this.env.CONFIG?.script_id, void 0);
      let hasStaticRouting = this.env.CONFIG.static_routing !== void 0, config = applyRouterConfigDefaults(this.env.CONFIG), eyeballConfig = applyEyeballConfigDefaults(this.env.EYEBALL_CONFIG), url = new URL(request.url);
      ...
      let routeToUserWorker = async ({
        ...
        return eyeballConfig.limitedAssetsOnly ? (analytics.setData({ userWorkerFreeTierLimiting: !0 }), new Response(renderLimitedResponse(request), {
        ...
        })) : (analytics.setData({ dispatchtype: "worker" /* WORKER */ }), userWorkerInvocation = !0, this.env.JAEGER.enterSpan("dispatch_worker", async (span) => {
          span.setTags({
            hasUserWorker: !0,
            asset,
          ...
          if (analytics.setData({
            ...
            let response = await this.env.USER_WORKER.fetch(request);
        ...
        asset
      }) => (analytics.setData({ dispatchtype: "asset" /* ASSETS */ }), await this.env.JAEGER.enterSpan(
        "dispatch_assets",
        async (span) => (span.setTags({
          hasUserWorker: config.has_user_worker,
          asset,
          dispatchType: "asset" /* ASSETS */
        }), analytics.setData({
          timeToDispatch: performance.now() - startTimeMs
        }), this.env.ASSET_WORKER.fetch(request))
      ));
      ...
      let assetsExist = await this.env.ASSET_WORKER.unstable_canFetch(
...
}, worker_default = RouterInnerEntrypoint;
